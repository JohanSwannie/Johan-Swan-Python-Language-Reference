...
# Print the middle Index of a LIST
# --------------------------------

lst111 = [1, 19, 101, 7, 15, 100, 188, 10, 21, 88, 92]
print('middle index of the list = ', lst111[(len(lst111)) // 2])
...
cities = ['New York', 'Miami', 'Auckland', 'Wellington', 'Sydney', 'Melbourne', 'Johannesburg']
print([x.upper() for x in cities])
print()

numbers = [15, 18, 21, 29, 33, 40, 65, 90, 101, 103, 110]
print([x for x in numbers if x % 2 == 0])
print()

numbers2 = range(0, 15)
print([x * x for x in numbers2 if x * x > 100])
print()

# LISTS with Functions
# ------------------

# Use a LIST to work out new salaries of employees after a rate increase

salary = ['Poepies', '62000', 'Mary', '112000', 'Johan', '94000', 'Peter', '170000']
increase = .05

def new_Salary(sal):
    for f in range(len(salary)):
        if f % 2 != 0:
            yield (int(sal[f]) * (1 + increase))
        else:
            yield sal[f]

ns = new_Salary(salary)
print(list(ns))
...
def new_Salary(sal):
    if sal.isdigit():
        return(int(sal) * (1 + increase))
    else:
        return sal

ns = map(new_Salary, salary)
print(list(ns))
